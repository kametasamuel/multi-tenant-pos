...
const ManagerInventory = ({ darkMode, surfaceClass, textClass, mutedClass, borderClass, bgClass }) => {
  ...
  const notifyOwner = async (product) => {
    ...
    try {
      ...
      showToast(`Owner notified about ${product.name}`);
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`px-6 py-3 ${darkMode ? 'bg-white text-black' : 'bg-slate-900 text-white'} rounded-2xl font-black text-[10px] uppercase shadow-lg hover:opacity-90 transition-opacity flex items-center gap-2`}
          >
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-48">
            ...
            <input
              ...
              placeholder="Search..."
              className={`w-full ${surfaceClass} border ${borderClass} rounded-xl py-2.5 pl-10 pr-4 text-xs font-bold focus:outline-none focus:border-accent-500 ${textClass}`}
            />
          </div>
          <button
          ...
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className={`${surfaceClass} border ${borderClass} rounded-[28px] p-5 shadow-sm hover:border-accent-500 transition-all group relative`}
                >
                  {/* Edit Button */}
                  <button
                    onClick={() => openEditModal(product)}
                    className={`absolute top-4 right-4 p-2 ${mutedClass} hover:text-accent-500 opacity-0 group-hover:opacity-100 transition-opacity`}
                  >
                  ...
                  <p className="font-black text-accent-500 text-sm">{formatCurrency(product.sellingPrice)}</p>
                  {product.costPrice > 0 && (
                  ...
                  {product.type === 'PRODUCT' ? (
                    <div className={`flex items-center gap-2 border-t pt-4 ${borderClass}`}>
                      <input
                        ...
                        className={`flex-1 ${darkMode ? 'bg-slate-800' : 'bg-slate-50'} border ${borderClass} rounded-xl p-2 text-center text-xs font-black focus:outline-none focus:border-accent-500 ${textClass} ${
                          isLowOrOut ? 'border-negative-300 bg-negative-50' : ''
                        }`}
                      />
                      <button
                        onClick={() => updateStock(product.id)}
                        className={`${darkMode ? 'bg-white text-black' : 'bg-slate-900 text-white'} p-2 rounded-xl hover:opacity-80 transition-opacity`}
                      >
                    ...
                    <div className={`border-t pt-4 ${borderClass}`}>
                      <p className={`text-[9px] ${mutedClass} uppercase text-center`}>Service - No Stock</p>
                    </div>
                  )}
                  ...
                  {isLowOrOut && product.type === 'PRODUCT' && (
                    <button
                      ...
                      className="w-full mt-3 flex items-center justify-center gap-2 py-2 bg-warning-500 text-white rounded-xl text-[9px] font-bold uppercase hover:bg-warning-600 transition-colors disabled:opacity-50"
                    >
                      <Bell className="w-3 h-3" />
                      {notifying === product.id ? 'Notifying...' : 'Alert Owner'}
                    </button>
                  )}
                </div>
              );
                  ...
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <tr key={product.id} className={`border-b ${borderClass} last:border-b-0 hover:bg-accent-50 ${darkMode ? 'hover:bg-slate-700/50' : ''}`}>
                        ...
                        <td className={`p-4 text-accent-500`}>{formatCurrency(product.sellingPrice)}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          {product.type === 'PRODUCT' ? (
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => updateStock(product.id)}
                                className="p-1.5 bg-accent-500 text-white rounded-lg hover:bg-accent-600"
                              >
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => openEditModal(product)}
                              className={`p-1.5 ${mutedClass} hover:text-accent-500`}
                              title="Edit"
                            >
                              <Edit3 className="w-4 h-4" />
                            </button>
                            {isLowOrOut && product.type === 'PRODUCT' && (
                              <button
                                onClick={() => notifyOwner(product)}
                                disabled={notifying === product.id}
                                className="p-1.5 text-warning-500 hover:text-warning-600 disabled:opacity-50"
                                title="Alert Owner"
                              >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {/* Profit Preview - only for products with cost price */}
                ...
                <>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full ${darkMode ? 'bg-slate-800' : 'bg-slate-50'} border ${borderClass} rounded-xl p-4 text-xs font-bold focus:outline-none focus:border-accent-500 ${textClass} cursor-pointer`}
              ...
              <button
                ...
                className={`w-full py-4 ${darkMode ? 'bg-white text-black' : 'bg-slate-900 text-white'} rounded-xl font-black text-[10px] uppercase shadow-lg mt-2`}
              >
                Add Item
              </button>
            </div>
