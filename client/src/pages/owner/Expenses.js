...
const OwnerExpenses = ({ darkMode, surfaceClass, textClass, mutedClass, borderClass, bgClass, currentBranch, isAllBranches, branches = [] }) => {
  const { user } = useAuth();
  ...
  const handleCreateExpense = async () => {
    ...
    if (isAllBranches && !newExpense.branchId) {
    ...
    try {
      ...
      if (isAllBranches) {
        expenseData.branchId = newExpense.branchId;
      } else if (currentBranch) {
        expenseData.branchId = currentBranch.id;
      }
      await expensesAPI.create(expenseData);
      setNewExpense({ description: '', amount: '', category: 'Utilities', branchId: '' });
  ...
  // Group expenses by actual date (not weekday) for better breakdown
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isAllBranches && (
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border ${borderClass} ${surfaceClass} ${textClass} text-xs font-bold uppercase hover:border-slate-400 transition-colors`}
              >
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 ${surfaceClass} border ${borderClass} rounded-xl text-xs font-bold uppercase ${textClass} hover:border-slate-400 transition-colors`}
            >
              ...
              <div className={`border-t ${borderClass} pt-4`}>
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-slate-800 dark:bg-slate-700 text-white rounded-xl text-xs font-bold uppercase disabled:opacity-50"
                  >
                    Apply Custom Range
                  </button>
                </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className={`${surfaceClass} border ${borderClass} p-8 sm:p-10 rounded-[40px] shadow-xl space-y-6`}>
          ...
          {isAllBranches && (
            <div>
              <label className={`text-[10px] font-black uppercase ${mutedClass} mb-1 block tracking-widest`}>Branch *</label>
              <select
          ...
          <button
            ...
            disabled={saving}
            className="w-full py-4 bg-slate-800 dark:bg-slate-700 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50"
          >
          ...
          <div className="max-h-[500px] overflow-y-auto space-y-3 pr-2" style={{ scrollbarWidth: 'thin' }}>
            {expenses.length === 0 ? (
              ...
              expenses.slice().reverse().map((expense) => (
                <div
                  key={expense.id}
                  className={`${surfaceClass} border ${borderClass} p-4 rounded-2xl flex justify-between items-center shadow-sm hover:border-slate-400 transition-colors`}
                >
                  <div>
